...
# by finding the crossmatches between GAIA and SQL query, can get the magnitude and 
# period values from VVV
...
def find_ID(GAIA_sourceID, results_ID, results_KsMaxMag, results_KsMinMag, results_KsMagRMS, results_Period, 
    ...
    respective measurementslike magnitudes, period, upper-lower-median distances. This is only
    done to stars that are 10kpc away, because distance error get larger after that point.
    ...
    GAIA_ID = np.array([])                              # defining empty arrays
    Ks_MaxMag = np.array([]) 
    Ks_MinMag = np.array([])                            # to fill them with the
    Ks_MagRMS = np.array([])                            # respective values 
    Ks_Period = np.array([])
    pixelID = np.array([])
...
def find_extinction(pixelID, GAIA_distance, extinction_data):
    ...
    """
    This function crossmatches pixel IDs and the distances from the 3D extinction map of VVV  which was made by
    Chen et. al (2013) to get the extinction values (colour excess) of the pixels, therefore relative stars. The 
    extinction measurements are listed in 5 kpc incriments until 10 kpc. The distances are converted to pc and since
    the stars have float distances, the extinction is found with the actual distance, the interval of distance and
    the relative extinction values. Mostly E(J-Ks) values are used, if they are no available E(H-Ks) is used. These
    values are multiplied by respective Nihiyama et al. (2009) extinction coefficients.
    ...
    Methods
    -------
    Mask is used to crossmatch pixel IDs of the stars to the 3D extinction maps. It returns boolean values; True
    if the pixel IDs match, False if they do not.
...
def find_xy(ejks_list, Ks_MaxMag, Ks_MinMag, Ks_MagRMS, GAIA_distance, Ks_Period, GAIA_lo, GAIA_hi):
    ...
    for k, l, m, n, o, p, q, r in zip(ejks_list, Ks_MaxMag, Ks_MinMag, Ks_MagRMS, GAIA_distance, Ks_Period, GAIA_lo, GAIA_hi):
        if k != 0.:
