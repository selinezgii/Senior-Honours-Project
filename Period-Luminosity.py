...
def find_ID(GAIA_sourceID, results_ID, results_KsMeanMag, results_KsMagRMS, results_Period, 
            results_pixelID, GAIA_r_med_photogeo, GAIA_r_lo_photogeo, GAIA_r_hi_photogeo, results_data): 
    ...
    measurementslike magnitudes, period, upper-lower-median distances. This is only done to stars that
    ...
    resultsAB_ID (column): Source IDs of the stars from VVV
    ...
    resultsAB_pixelID (column): Pixel IDs of stars chosen from VVV
...
# finding extinction of the pixels
...
def find_xy(ejks_list, Ks_MeanMag, GAIA_distance, Ks_Period, Ks_MagRMS, GAIA_lo, GAIA_hi):
    ...
    for k, l, m, n, o, p, q in zip(ejks_list, Ks_MeanMag, GAIA_distance, Ks_Period, Ks_MagRMS, GAIA_lo, GAIA_hi):
        ...
        if k != 0.:
         ...
         litmag.append(((-0.597 ) - (2.353 * np.log10(n) + (0.175 * 0.001))))
...
def main():
    ...
    # RRAB stars
    ABGAIA_ID, ABKs_MeanMag, ABKs_MagRMS, ABKs_Period, ABpixelID, ABGAIA_distance, ABGAIA_lo, ABGAIA_hi  = find_ID(GAIA_sourceID, resultsAB_ID, 
                                                resultsAB_KsMeanMag, resultsAB_KsMagRMS, 
                                                resultsAB_Period, resultsAB_pixelID, GAIA_r_med_photogeo, GAIA_r_lo_photogeo, GAIA_r_hi_photogeo, resultsAB_data)
    
    ABejks_list = np.array([find_extinction(pixID, gaiaDist, extinction_data) for pixID,gaiaDist in zip(ABpixelID, ABGAIA_distance)])
    
    ABx, ABy, AByerrup, AByerrdown, ABlitmag = find_xy(ABejks_list, ABKs_MeanMag, ABGAIA_distance, ABKs_Period, ABKs_MagRMS, ABGAIA_lo, ABGAIA_hi)
    
    
    
    # RRC stars
    CGAIA_ID, CKs_MeanMag, CKs_MagRMS, CKs_Period, CpixelID, CGAIA_distance, CGAIA_lo, CGAIA_hi  = find_ID(GAIA_sourceID, resultsC_ID, 
                                                resultsC_KsMeanMag, resultsC_KsMagRMS, 
                                                resultsC_Period, resultsC_pixelID, GAIA_r_med_photogeo, GAIA_r_lo_photogeo, GAIA_r_hi_photogeo, resultsAB_data)
    
    Cejks_list = np.array([find_extinction(pixID, gaiaDist, extinction_data) for pixID,gaiaDist in zip(CpixelID, CGAIA_distance)])
    
    Cx, Cy, Cyerrup, Cyerrdown, Clitmag = find_xy(Cejks_list, CKs_MeanMag, CGAIA_distance, CKs_Period, CKs_MagRMS, CGAIA_lo, CGAIA_hi)
    ...
    plt.errorbar(ABx, ABy, yerr = [AByerrdown, AByerrup], ecolor = 'red', elinewidth = 0.5, alpha = 0.3, capsize = 1.5, linewidth = 0)
    
    # for RRC
    plt.scatter(Cx, Cy, s = 5, alpha = 0.3, color = "blue", label = "RRC")
    plt.errorbar(Cx, Cy, yerr = [Cyerrdown, Cyerrup], ecolor = 'blue', elinewidth = 0.5, alpha = 0.3, capsize = 1.5, linewidth = 0)
